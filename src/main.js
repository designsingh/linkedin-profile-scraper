...
async function enqueueHtmlFallback(slug, profileUrl, reason) {
        ...
        log.warning(`[${slug}] ${reason} — falling back to HTML scraping`);
...
const crawler = new CheerioCrawler({
        ...
        preNavigationHooks: [
                    async ({ request }) => {
                        ...
                        } else if (cookie && !request.userData.useApi) {
        ...
        async requestHandler({ request, body, response, session }) {
            ...
            // FIX: API auth failures → fall back to HTML instead of blind retry
            ...
            if (useApi) {
                        ...
                        // FIX: If API returned data but parse was empty, fall back to HTML
                        if (!profile || !profile.fullName) {
                                            await enqueueHtmlFallback(slug, profileUrl, `Voyager API returned empty profile (name: "${profile?.fullName || ''}")`);
                                            return;
                        }
            } else {
                            // ── Parse HTML page ────────────────────────────────────────
        ...
        // FIX: Fall back to HTML on redirect/network errors for API requests
        async failedRequestHandler({ request }, error) {
                    const { slug, profileUrl, useApi } = request.userData;

            // If the API path failed completely (e.g. MaxRedirectsError), try HTML
            if (useApi) {
                            const enqueued = await enqueueHtmlFallback(slug, profileUrl, `API path failed: ${error.message}`);
